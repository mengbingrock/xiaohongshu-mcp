...
func setupRoutes(appServer *AppServer) *gin.Engine {
	...
	api := protected.Group("/api/v1")
	{
		...
		api.GET("/login/sessions/:session_id", appServer.getLoginSessionStatusHandler)
		api.POST("/login/code", appServer.submitLoginCodeHandler)
		api.DELETE("/login/cookies", appServer.deleteCookiesHandler)
		api.POST("/chineseinla/login/start", appServer.startChineseInLALoginHandler)
		api.GET("/chineseinla/login/sessions/:session_id", appServer.getChineseInLALoginSessionHandler)
		api.POST("/chineseinla/login/password", appServer.submitChineseInLALoginPasswordHandler)
